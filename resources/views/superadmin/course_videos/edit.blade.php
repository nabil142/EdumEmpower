            ...
            <div class="bg-white overflow-hidden p-10 shadow-sm sm:rounded-lg">
                ...
                <div class="item-card flex flex-row gap-y-10 justify-between items-center">
                    ...
                    <div>
                        ...
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $courseVideo->course->admin->user->name }}</h3>
                    </div>
                </div>

                <hr class="my-5">
                
                <form method="POST" action="{{ route('superadmin.course_videos.update', $courseVideo) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <div>
                        <x-input-label for="name" :value="__('Name')" />
                        <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" value="{{$courseVideo->course->name}}" required autofocus autocomplete="name" />
